(() => {
  ...
  function installStyles(){
    ...
    if(document.getElementById('projectToolbarV4Style'))return;
    const style=document.createElement('style');style.id='projectToolbarV4Style';style.textContent=`
      ...
      .project-save-primary{display:inline-flex;align-items:center;gap:7px;white-space:nowrap;flex:0 0 auto}.project-save-icon{font-size:15px;line-height:1}
      ...
      @media(max-width:780px){.topbar{gap:6px;padding-inline:8px}.top-actions{gap:4px}.brand-sub{display:none}.project-save-primary{width:40px;min-width:40px;height:38px;padding:0;justify-content:center;border-radius:10px}.project-save-primary .project-save-label{display:none}.project-save-primary .project-save-icon{font-size:18px}.project-menu-btn{width:38px;min-width:38px;height:38px;padding:0}.project-popover-fixed{min-width:180px}.stage-toolbar{gap:6px}.stage-actions{gap:5px}.stage-actions .mini-btn{white-space:nowrap}}
      ...
      html.phone-layout .project-save-primary{width:40px;min-width:40px;height:38px;padding:0;justify-content:center}html.phone-layout .project-save-primary .project-save-label{display:none}html.phone-layout .project-menu-btn{width:38px;min-width:38px;height:38px;padding:0}
  ...
  function toggleProjectMenu(e){
    ...
    const willOpen=!menu.classList.contains('open');closeProjectMenu();if(willOpen){menu.classList.add('open');btn.setAttribute('aria-expanded','true');requestAnimationFrame(positionProjectMenu);}
  }
  ...
  function rebuildToolbar(){
    ...
    const save=document.createElement('button');save.type='button';save.id='saveProjectPrimaryBtn';save.className='btn btn-primary project-save-primary';save.innerHTML='<span class="project-save-icon" aria-hidden="true">▣</span><span class="project-save-label"></span>';save.addEventListener('click',()=>S.exportProject?.());actions.appendChild(save);
    ...
    btn.addEventListener('pointerup',e=>{if(e.pointerType==='touch'||e.pointerType==='pen'){toggleProjectMenu(e);}});
    ...
    document.addEventListener('pointerdown',e=>{if(!menu.contains(e.target)&&e.target!==btn)closeProjectMenu();},{capture:true});
    document.addEventListener('keydown',e=>{if(e.key==='Escape')closeProjectMenu();});
    window.addEventListener('scroll',closeProjectMenu,{passive:true});
  }
  ...
  function localizeNewUI(){
    const save=document.getElementById('saveProjectPrimaryBtn'),menuBtn=document.getElementById('projectMenuBtn');if(save){save.querySelector('.project-save-label').textContent=tr('save');save.title=tr('save');save.setAttribute('aria-label',tr('save'));}if(menuBtn){menuBtn.title=tr('projectMenu');menuBtn.setAttribute('aria-label',tr('projectMenu'));}
