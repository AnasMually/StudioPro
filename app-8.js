(() => {
  ...
  const T={
    ...
    es:{save:'Guardar proyecto',import:'Importar proyecto',exportPng:'Exportar PNG',projectMenu:'Opciones del proyecto',undo:'Deshacer',redo:'Rehacer',imageShape:'Forma de imagen',edgeStyle:'Estilo de bordes',original:'Predeterminado / sin recorte extra',rounded:'Esquinas redondeadas',squircle:'Squircle — estilo One UI',smooth:'Suavidad Squircle',smoothHelp:'Distribuye la curvatura de forma continua por los bordes, no solo en las esquinas.'},
    ...
    'zh-CN':{save:'保存项目',import:'导入项目',exportPng:'导出 PNG',projectMenu:'项目选项',undo:'撤销',redo:'重做',imageShape:'图片形状',edgeStyle:'边缘样式',original:'默认 / 不额外裁剪',rounded:'圆角',squircle:'Squircle — One UI 风格',smooth:'Squircle 平滑度',smoothHelp:'让曲率沿整个边缘连续分布，而不是只集中在四个角。'}
  };
  ...
  function installStyles(){
    if(document.getElementById('projectToolbarV3Style'))return;
    document.getElementById('projectToolbarV2Style')?.remove();
    const style=document.createElement('style');style.id='projectToolbarV3Style';style.textContent=`
      html,body,.app{max-width:100%;overflow-x:hidden}
      .topbar,.workspace-grid,.stage-column,.stage-toolbar{max-width:100%}
      .top-actions{min-width:0;flex:0 0 auto}
      .brand-wrap{min-width:0;overflow:hidden}.brand-wrap>div:last-child{min-width:0}.brand,.brand-sub{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .project-menu-wrap{position:relative;display:inline-flex;align-items:center;flex:0 0 auto}
      ...
      .project-save-primary{display:inline-flex;align-items:center;gap:7px;white-space:nowrap;flex:0 0 auto}
      .project-save-icon{font-size:15px;line-height:1}
      .stage-history-btn{width:30px;height:30px;border-radius:8px;background:#fff;border:1px solid var(--border);color:var(--text);display:grid;place-items:center;font-size:15px;cursor:pointer;padding:0}
      .stage-history-btn:disabled{opacity:.35;cursor:default}
      .stage-actions{min-width:0;flex-wrap:nowrap}
      .stage-status{min-width:0;white-space:nowrap}

      @media(max-width:780px){
        .topbar{gap:6px;padding-inline:8px}.top-actions{gap:4px}.brand-sub{display:none}
        .project-save-primary{width:40px;min-width:40px;height:38px;padding:0;justify-content:center;border-radius:10px}
        .project-save-primary .project-save-label{display:none}.project-save-primary .project-save-icon{font-size:18px}
        .project-menu-btn{width:38px;min-width:38px;height:38px;padding:0}
        .project-popover{min-width:180px}
        .stage-toolbar{gap:6px}.stage-actions{gap:5px}.stage-actions .mini-btn{white-space:nowrap}
      }

      /* Force the real mobile layout on touch phones even when the browser exposes a desktop-sized viewport. */
      html.phone-layout body{overflow:hidden!important}
      html.phone-layout .desktop-only{display:none!important}
      html.phone-layout .mobile-only{display:flex!important}
      ...
      html.phone-layout .stage-column{height:100%;width:100%!important;min-width:0!important;grid-template-rows:42px minmax(0,1fr)!important}
      html.phone-layout .stage{padding:10px;max-width:100%;overflow:hidden}
      html.phone-layout .stage-toolbar{padding:0 8px;width:100%;overflow:hidden}
      ...
      html.phone-layout .panel.mobile-open{transform:translateY(0)!important;opacity:1!important;pointer-events:auto!important}
      html.phone-layout .mobile-dock{display:flex!important;position:fixed;left:0;right:0;bottom:0;height:calc(66px + env(safe-area-inset-bottom));z-index:80}
      html.phone-layout .brand-sub{display:none!important}
      html.phone-layout .topbar{height:58px;padding-inline:8px;gap:5px;width:100%;overflow:hidden}
      html.phone-layout .brand-mark{width:34px;height:34px;flex:0 0 auto}
      html.phone-layout .brand{font-size:14px}
      html.phone-layout .project-save-primary{width:40px;min-width:40px;height:38px;padding:0;justify-content:center}
      html.phone-layout .project-save-primary .project-save-label{display:none}
      html.phone-layout .project-menu-btn{width:38px;min-width:38px;height:38px;padding:0}
      html.phone-layout .stage-status{font-size:9px;flex:0 1 auto;overflow:hidden;text-overflow:ellipsis}
      html.phone-layout .stage-actions{flex:0 0 auto;gap:4px}
      html.phone-layout .stage-actions .mini-btn{padding:5px 6px;font-size:9px}
      html.phone-layout .stage-history-btn{width:29px;height:29px;font-size:14px}

      @media(max-width:390px){.brand-wrap{min-width:0}.brand{font-size:13px}.brand-mark{width:31px;height:31px}.topbar{padding-inline:6px}.top-actions{gap:3px}.stage-status{max-width:82px}}
  ...
  function rebuildToolbar(){
    ...
    /* PNG must exist only inside the project menu. Remove the old prominent button completely. */
    document.getElementById('exportBtn')?.remove();
    document.getElementById('legacyExportPngHidden')?.remove();
  ...
  function relocateHistoryControls(){
    const stageActions=document.querySelector('.stage-actions');if(!stageActions)return;
  ...
  window.addEventListener('resize',()=>{markPhoneLayout();requestAnimationFrame(()=>S.fitCanvasToStage?.());});
  window.addEventListener('orientationchange',()=>setTimeout(()=>{markPhoneLayout();S.fitCanvasToStage?.();},120));
})();