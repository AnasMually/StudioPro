(() => {
  ...
  L.makePrompt=()=>{
    ...
    return`Act as a senior mobile-app localization specialist for Google Play marketing screenshots.\n\nTranslate every source item independently into every target locale listed below.\n\nSOURCE ITEMS (IDs are immutable and must be returned exactly):\n${JSON.stringify(items,null,2)}\n\nTARGET LOCALES — use these exact locale codes as JSON keys:\n${targets.map(c=>`${c} (${L.name(c)})`).join(', ')}\n\nRULES:\n1. Return ONLY valid JSON. No Markdown, no code fences, no explanations.\n2. Do not merge, remove, rename, reorder, or invent text IDs.\n3. Translate each item as a separate marketing string while preserving its original meaning and tone.\n4. Localize for the exact requested locale, not merely the base language. Preserve meaningful regional spelling, vocabulary, punctuation, script, and conventions.\n5. Keep regional variants distinct. For example, en-US/en-GB/en-AU/en-CA/en-IN/en-SG, fr-FR/fr-CA, pt-BR/pt-PT, es-419/es-ES/es-US, and zh-CN/zh-TW/zh-HK must each be localized appropriately rather than copied blindly.\n6. Keep wording natural, concise, and suitable for a mobile app screenshot. Do not add claims, prices, rankings, awards, features, or facts not present in the source.\n7. Preserve deliberate line breaks when meaningful. Encode them as \\n inside JSON strings.\n8. Every requested locale must contain every source ID exactly once.\n9. Use ONLY the exact locale codes supplied above under "translations". Do not substitute aliases (for example, keep iw-IL exactly as requested).\n10. Do not return any locale that was not requested.\n\nREQUIRED RESPONSE SCHEMA:\n${JSON.stringify(example,null,2)}\n\nThe complete response must use:\n{"schema_version":1,"translations":{"<locale-code>":{"<text-id>":"translated text"}}}`;
  };
